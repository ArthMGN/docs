...
| **Nome**         | **Tipo** | **Descrição**                                                                                                                                                                  |
| :--------------- | :------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `p_destino`      | string   | Identificador lógico do sistema consumidor da integração.<br />Usado para controle de idempotência (garante que a mesma NF não seja processada duas vezes pelo mesmo sistema). |
| `p_ag_documento` | string   | Documento (CNPJ/CPF) do AG (armazém geral) a ser filtrado                                                                                                                      |
| `p_limit`        | int      | Quantidade máxima de notas fiscais retornadas na consulta.                                                                                                                     |
...
{
  "p_destino": "ERP",
  "p_ag_documento": "12.345.678/0001-99",
  "p_limit": 50
}
```
